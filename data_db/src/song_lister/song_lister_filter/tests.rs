...
mod test_group_filter {
    ...
    async fn complex_and_or_combination(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target.list_song_id(&mut tx, &filter).await?;

        // song_ids: 1, 3, 6, 7 が該当するはず
...
mod test_int_filter {
    ...
    async fn equal(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(IntFilterRange::Equal { value: 9 }))
            .await?;
    ...
    async fn not_equal(pool: PgPool) -> anyhow::Result<()> {
        ...
        let result = target
            .list_song_id(&mut tx, &filter(IntFilterRange::NotEqual { value: 25 }))
            .await?;
    ...
    async fn large_equal(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(IntFilterRange::LargeEqual { value: 10 }))
            .await?;
    ...
    async fn small_equal(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(IntFilterRange::SmallEqual { value: 5 }))
            .await?;
...
mod test_tags_filter {
    ...
    async fn contain_existing_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::Contain { value: 4 }))
            .await?;
    ...
    async fn not_contain_existing_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::NotContain { value: 4 }))
            .await?;
    ...
    async fn contain_nonexistent_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::Contain { value: 5 }))
            .await?;
    ...
    async fn not_contain_nonexistent_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::NotContain { value: 5 }))
            .await?;
    ...
    async fn contain_another_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::Contain { value: 83 }))
            .await?;
    ...
    async fn not_contain_another_tag(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::NotContain { value: 83 }))
            .await?;
    ...
    async fn none(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(TagsFilterRange::None))
            .await?;
...
mod test_bool_filter {
    ...
    async fn is_true(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(BoolFilterRange::True))
            .await?;
    ...
    async fn is_false(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(BoolFilterRange::False))
            .await?;
...
mod test_artwork_filter {
    ...
    async fn has_artwork(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(ArtworkFilterRange::Has))
            .await?;
    ...
    async fn no_artwork(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(ArtworkFilterRange::None))
            .await?;
...
mod test_date_filter {
    ...
    async fn none(pool: PgPool) -> anyhow::Result<()> {
        ...
        let target = target();
        let result = target
            .list_song_id(&mut tx, &filter(DateFilterRange::None))
            .await?;
